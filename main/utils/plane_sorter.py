...
def transform(points:list, draw_plane:str,extrusion:str='0',zerodisplacement:str='0'):
    ...
    match draw_plane:
        ...
        case 'yz':
            ...
            extruded_plane = []
            for point in points:
                original_plane.append([zerodisplacement,point[1],point[0]])
                extruded_plane.append([extrusion+zerodisplacement,point[1],point[0]])
            plane_array.append(original_plane)
            plane_array.append(extruded_plane)
            ...
            for i in range(1,len(points)):
                side_plane = []
                side_plane.extend([[zerodisplacement,points[i][1],points[i][0]], [extrusion+zerodisplacement,points[i][1],points[i][0]], [extrusion+zerodisplacement,points[i-1][1],points[i-1][0]], [zerodisplacement,points[i-1][1],points[i-1][0]]])
                plane_array.append(side_plane)
            side_plane = []
            side_plane.extend([[zerodisplacement,points[-1][1],points[-1][0]], [extrusion+zerodisplacement,points[-1][1],points[-1][0]], [extrusion+zerodisplacement,points[0][1],points[0][0]], [zerodisplacement,points[0][1],points[0][0]]])
            plane_array.append(side_plane)
